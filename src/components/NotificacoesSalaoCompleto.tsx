...
export default function NotificacoesDonoPage() {
  ...
  // Atualiza a Central periodicamente para que um atendimento entre
  // automaticamente na aba Confirmar assim que o horário chegar.
  useEffect(() => {
    if (loading || !profile?.salao_id) return
  ...
  async function registrarPushNotification() {
    ...
    try {
      ...
      // A tabela possui UNIQUE em profile_id, não em user_id.
      // Atualizamos somente a associação desta conta com este dispositivo.
      ...
      if (erroBusca) {
        console.error(
          'Erro ao localizar associação de Push da conta:',
          erroBusca
        )
        return
      }
      ...
      if (existente?.id) {
        ...
        if (error) {
          console.error(
            'Erro ao atualizar associação de Push:',
            error
          )
        }
      } else {
        ...
        if (error) {
          console.error(
            'Erro ao criar associação de Push:',
            error
          )
        }
      }
  ...
  // Atualiza as notificações do dono em tempo real.
  // O filtro usa o profile.id para que uma conta nunca receba
  // atualizações destinadas a outro usuário.
  useEffect(() => {
    if (loading || !profile?.id || !profile?.salao_id) {
      return
    }

    const canal = supabase
      .channel(
        `notificacoes-dono-${profile.id}`
      )
      .on(
        'postgres_changes',
        {
  ...
  async function carregarDados() {
    ...
    const { data: sols } = await supabase
      ...
      .select(
        '*, clientes(id, profile_id, nome, email, telefone), servicos(nome, duracao_minutos)'
      )
      .eq('salao_id', profile.salao_id)
      .in('status', ['pendente', 'horario_sugerido'])
      .order('created_at', { ascending: false })
    setSolicitacoes(sols || [])
    // O atendimento só entra na aba Confirmar quando o horário
    // agendado já chegou. Antes disso, permanece apenas na Agenda.
    const { data: ags } = await supabase
      .from('agendamentos')
      .select(
        '*, clientes(id, profile_id, nome, telefone), servicos(nome, id), confirmacoes_atendimento(*)'
      )
      .eq('salao_id', profile.salao_id)
    ...
    setConfirmacoes(
      (ags || []).filter(
        (a: any) => {
          if (a.confirmacoes_atendimento?.length) {
            return false
          }
          ...
          if (Number.isNaN(horarioAgendamento)) {
            return false
          }
  ...
  async function abrirModalConfirmar(
    ...
    try {
      ...
      // Monta os serviços do próprio agendamento para que a confirmação
      // mostre todos os serviços realizados, e não apenas o primeiro.
      const dadosServicos =
  ...
  async function confirmarAtendimento() {
    ...
    try {
      ...
      await notificar({
        ...
        destinatarioId:
          agendamento.clientes?.profile_id ||
          null,
        url: '/cliente'
      })
  ...
  async function aceitarSolicitacao(
    ...
    if (!dataHora) {
      alert(
        solicitacao?.status === 'horario_sugerido'
          ? 'A cliente ainda precisa escolher um dos horários enviados. Quando ela escolher, você poderá aceitar o pedido.'
          : 'Esta solicitação não possui um horário definido. Use “Sugerir horários” para enviar horários disponíveis antes de aceitar.'
      )
      return
    }
    ...
    if (
      ...
      alert(
        'O horário desta solicitação é inválido. Use “Sugerir horários” para definir um horário válido.'
      )
      return
    }
    ...
    try {
      ...
      await notificar({
        ...
        destinatarioId:
          solicitacao.clientes?.profile_id ||
          null,
        url: '/cliente'
      })
  ...
  return (
    ...
    >
      <span className="flex items-center justify-center gap-2">
        ...
        {quantidade > 0 && (
          <span
            className={`min-w-5 h-5 px-1.5 rounded-full text-[11px] flex items- center justify-center ${               ativo                 ? 'bg-white/20 text-white'                 : 'bg-[#f5f5f5] text-[var(--cor-salao)]'             } ${               label === 'Notificações' && quantidade > 0 ? 'animate-pulse font-bold' : ''             }`}
          >
            {quantidade}
          </span>
        )}
            ...
            <div>
              <div className="rounded-2xl bg-[#f8f8f8] p-4">
                ...
                <textarea
                  ...
                  className="w-full mt-2 min-h-[90px] rounded-xl border border-[#e2e2e2] bg-white px-3 py-3 text-sm outline-none resize-none"
                  placeholder="Nome dos serviços"
                />
              </div>
              {verificandoPacoteConfirmacao ? (
