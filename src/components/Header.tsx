...
const MAPA_ROTAS_SISTEMA: Record<
  string,
  {
    label: string
    href: string
...
> = {
  ...
  contratos: {
    label: 'Contratos',
    href: '/salao/contratos',
    grupo: 'Atendimento',
    icon: FileText
  },

  horarios_vagos: {
    label: 'Horários vagos e de funcionamento',
    href: '/salao/horarios-vagos',
    grupo: 'Atendimento',
    icon: Clock
  },
  ...
  quem_somos: {
    label: 'Quem Somos',
    href: '/salao/quem-somos',
    grupo: 'Outros',
    icon: Heart
  },

  avaliacoes: {
    label: 'Avaliações e Prints',
...
export default function Header({
  ...
  } = useNotificacoes(profile.id)

  const [menuAberto, setMenuAberto] = useState(false)
  const [notifAberta, setNotifAberta] = useState(false)
  const [menuFuncionarioDinamico, setMenuFuncionarioDinamico] =
    useState<any[]>([])
  ...
  useEffect(() => {
    async function carregarPermissoesFuncionario() {
      ...
      if (cargosFuncionario.includes(userRole)) {
        ...
        // Se não houver permissões específicas para o usuário,
        // tenta carregar as permissões do cargo.
        ...
        if (permissoesDB && permissoesDB.length > 0) {
          ...
          permissoesDB.forEach((p: any) => {
            if (
              p.permitido === true &&
              MAPA_ROTAS_SISTEMA[p.pagina_key]
            ) {
              const configRota =
                MAPA_ROTAS_SISTEMA[p.pagina_key]

              if (
                p.pagina_key !== 'dashboard' &&
                !rotasAdicionadas.has(configRota.href)
              ) {
          ...
          // Notificações continuam disponíveis para funcionários.
  ...
  const menuItems =
    userRole === 'admin_geral'
      ...
      : [
          ...
          'comum',
          'socio'
        ].includes(userRole) && profile?.salao_id
      ? menuFuncionarioDinamico
      : MENU_DONO

  const grupos = Array.from(
    new Set(menuItems.map(i => i.grupo))
  )
  ...
  async function handleClicarNotificacao(n: any) {
    ...
    if (!destino) {
      ...
      const clienteId =
        n.cliente_id ||
        n.dados_extras?.cliente_id

      if (
        tipo === 'duplicado' ||
        titulo.includes('duplicado')
      ) {
        destino = clienteId
          ? `/salao/clientes?mesclar=${clienteId}`
          : '/salao/clientes'
      } else if (
        tipo === 'agendamento' ||
