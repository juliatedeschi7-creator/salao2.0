...
export function useNotificacoes(profileId?: string) {
  ...
  useEffect(() => {
    if (!profileId) return
    buscar()
    const canal = supabase.channel('notif-' + profileId)
      .on('postgres_changes', {
  ...
  async function buscar() {
    const { data } = await supabase.from('notificacoes').select('*')
      .eq('destinatario_id', profileId).order('created_at', { ascending: false }).limit(50)
  ...
  async function marcarTodasComoLidas() {
    await supabase.from('notificacoes').update({ lida: true }).eq('destinatario_id', profileId).eq('lida', false)
