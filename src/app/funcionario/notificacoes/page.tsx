...
export default function FuncionarioNotificacoesPage() {
  ...
  async function carregarDados() {
    ...
    setSalao(sal)
    const { data: notifs } = await supabase.from('notificacoes').select('*').eq('destinatario_id', profile.id).order('created_at', { ascending: false })
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-4 flex flex-col gap-3">
        {notificacoes.length === 0 ? (
          <div className="card text-center py-10"><Bell size={36} className="text-gray-300 mx-auto mb-2" /><p className="text-gray-400">Nenhuma notificacao</p></div>
        ) : notificacoes.map(n => (
          <div key={n.id} className={'card flex flex-col gap-1 ' + (!n.lida ? 'border-l-4' : '')} style={!n.lida ? { borderColor: cor } : {}}>
