...
export default function NovoAgendamentoPage() {
  ...
  const [frequenciaFixo, setFrequenciaFixo] = useState<'semanal' | 'quinzenal'>('semanal')
  ...
  async function handleSalvar() {
    ...
    // Se for horário fixo, gera as próximas ocorrências (semanal = 7 em 7 dias,
    // quinzenal = 14 em 14 dias). Todas compartilham o mesmo recorrencia_id,
    // mas cada uma é uma linha independente — dá pra cancelar uma sem afetar as outras.
    const TOTAL_OCORRENCIAS = horarioFixo ? 8 : 1
    const intervaloDias = frequenciaFixo === 'quinzenal' ? 14 : 7
    ...
    const agendamentosParaCriar = Array.from({ length: TOTAL_OCORRENCIAS }, (_, i) => {
      const dataOcorrencia = new Date(`${data}T${hora}:00`)
      dataOcorrencia.setDate(dataOcorrencia.getDate() + i * intervaloDias)
          ...
          {horarioFixo && (
            <div className="flex flex-col gap-2 pl-[52px]">
              <div className="flex gap-2">
                <button onClick={() => setFrequenciaFixo('semanal')}
                  className="flex-1 py-2 rounded-lg text-sm font-medium border-2 transition-all"
                  style={frequenciaFixo === 'semanal'
                    ? { borderColor: cor, backgroundColor: `${cor}10`, color: cor }
                    : { borderColor: '#e5e7eb', color: '#9ca3af' }}>
                  Semanal
                </button>
                <button onClick={() => setFrequenciaFixo('quinzenal')}
                  className="flex-1 py-2 rounded-lg text-sm font-medium border-2 transition-all"
                  style={frequenciaFixo === 'quinzenal'
                    ? { borderColor: cor, backgroundColor: `${cor}10`, color: cor }
                    : { borderColor: '#e5e7eb', color: '#9ca3af' }}>
                  Quinzenal
                </button>
              ...
              <p className="text-xs text-gray-400">
                Serão criadas as próximas 8 ocorrências. Você pode cancelar uma data específica depois, direto na agenda, sem afetar as demais.
              </p>
            </div>
          )}
        </div>
