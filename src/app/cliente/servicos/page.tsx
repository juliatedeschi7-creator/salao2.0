...
function AvisoServicos({
  ...
  function renderLinha(linha: string) {
    ...
    return palavras.map((palavra, i) => {
      ...
      return (
        <span key={i} className={ehMaiuscula ? 'font-bold' : ''}>
          {palavra}{' '}
        </span>
      )
    })
...
export default function ClienteServicosPage() {
  ...
  const filtrados = servicos.filter(
    s => categoriaFiltro === 'Todos' || s.categoria === categoriaFiltro
  )
  ...
  function temAlerta(descricao: string) {
    ...
    return ALERTAS.some(a =>
      descricao.toLowerCase().includes(a)
    )
  }
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-yellow-600 mt-0.5">
                          Este servico pode ter restricoes para certas
                          condicoes de saude. Informe ao profissional
                          sobre diabetes, alergias, gestacao ou outras
                          condicoes antes do procedimento.
                        </p>
                      </div>
                    </div>
                  )}
