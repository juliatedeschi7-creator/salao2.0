...
export default function ClientesPage() {
  ...
  const filtrados = clientes.filter(c => c.nome.toLowerCase().includes(busca.toLowerCase()) || c.telefone?.includes(busca) || c.email?.toLowerCase().includes(busca.toLowerCase()))
  const navItems = [
    ...
    <div className="min-h-screen pb-20" style={{ backgroundColor: '#f8f9fa' }}>
      <div className="bg-white px-4 py-4 flex items-center gap-3 shadow-sm">
        ...
        <button onClick={() => router.push('/salao/clientes/novo')}
          className="w-9 h-9 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: cor }}>
          <Plus size={18} />
        </button>
      </div>
      <div className="px-4 py-4 flex flex-col gap-3">
        <div className="card flex flex-col gap-2" style={{ backgroundColor: salao?.cor_secundaria || '#FCE4F3' }}>
          <p className="text-sm font-semibold" style={{ color: cor }}>Link de convite para clientes</p>
          <p className="text-xs text-gray-500">Compartilhe para suas clientes se cadastrarem</p>
          <button onClick={copiarLink} className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: cor }}>
        ...
        <p className="text-xs text-gray-400">{filtrados.length} cliente(s)</p>

        {filtrados.length === 0 ? (
          <div className="card text-center py-10"><User size={36} className="text-gray-300 mx-auto mb-2" /><p className="text-gray-400">Nenhuma cliente encontrada</p></div>
        ) : filtrados.map(c => (
          <button key={c.id} onClick={() => router.push('/salao/clientes/' + c.id)}
