...
export default function EditarAgendamentoPage() {
  ...
  async function carregarAgendamento() {
    ...
    // ← Protege contra servicos_ids/profissionais_ids ainda nulos no banco
    const srvIds = Array.isArray(agend.servicos_ids) && agend.servicos_ids.length > 0
      ? agend.servicos_ids
      : agend.servico_id ? [agend.servico_id] : []

    const profIds = Array.isArray(agend.profissionais_ids) && agend.profissionais_ids.length > 0
      ? agend.profissionais_ids
      : agend.profissional_id ? [agend.profissional_id] : []

    setServicosSelecionados(srvIds)
  ...
  return (
    <div className="min-h-screen bg-[#f8f4f6]">
      <div className="bg-white px-4 py-4 flex items-center justify-between shadow-sm">
        ...
        <button onClick={() => setMostrarModalExclusao(true)} className="p-2 text-red-500">
          <Trash2 size={20} />
        </button>
      </div>
      ...
      <div className="px-4 py-4 flex flex-col gap-4 pb-8">
        ...
        <div>
            ...
            <div className="mb-3 flex gap-2 flex-wrap">
              <button onClick={() => setCategoriaFiltro(null)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                  categoriaFiltro === null ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-700'
                }`}>Todas</button>
              {categorias.map(cat => (
                ...
                <button key={s.id} onClick={() => toggleServico(s.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                      ...
                      <span>·</span><span>R$ {Number(s.preco).toFixed(2)}</span>
                    </div>
                  </div>
                </button>
              )
          ...
          {servicosSelecionados.length > 0 && (
            <div className="mt-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
              ...
              <p className="text-sm font-semibold text-gray-900">Duração: {formatarDuracao(duracaoTotal)}</p>
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 mb-1.5 block">
            PROFISSIONAIS {profissionaisSelecionados.length > 0 && `(${profissionaisSelecionados.length})`}
          </label>
          <div className="flex flex-col gap-2">
            {profissionais.map(p => {
          ...
          {profissionaisSelecionadosInfo.length > 0 && (
            <div className="mt-3 p-3 rounded-xl border flex flex-wrap gap-2"
              style={{ backgroundColor: `${cor}08`, borderColor: `${cor}30` }}>
              {profissionaisSelecionadosInfo.map(p => (
                <span key={p.id}
                  className="inline-flex items-center gap-1.5 px-2 py-1 bg-white rounded-lg text-xs font-medium text-gray-900 border"
                  style={{ borderColor: cor }}>
                  {p.nome}
                  <button onClick={() => toggleProfissional(p.id)} className="text-gray-400">
                    <X size={12} />
                  </button>
                </span>
              ))}
        ...
        <div className="card flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: `${cor}18` }}>
      ...
      {mostrarModalExclusao && (
        <div className="fixed inset-0 bg-black/50 flex items-end z-50">
          <div className="w-full bg-white rounded-t-3xl p-6 flex flex-col gap-3">
            ...
            <button onClick={handleExcluir} disabled={excluindo}
              className="w-full py-3 bg-red-500 text-white font-semibold rounded-xl disabled:opacity-50">
              {excluindo ? 'Cancelando...' : 'Sim, cancelar agendamento'}
            </button>
            <button onClick={() => setMostrarModalExclusao(false)}
              className="w-full py-3 bg-gray-100 text-gray-900 font-semibold rounded-xl">
              Não, voltar
            </button>
          </div>
        </div>
