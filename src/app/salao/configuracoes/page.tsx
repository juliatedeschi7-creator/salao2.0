...
import { ArrowLeft, Bell, Link, Copy, Check, LogOut, Palette, Smartphone } from 'lucide-react'
...
export default function ConfiguracoesPage() {
  ...
  async function carregarDados() {
    ...
    if (sal?.modulos_cliente) {
      setModulos({ ...modulos, ...sal.modulos_cliente })
    }
  }
  ...
  async function enviarTesteNotificacao() {
    ...
    await supabase.from('notificacoes').insert({
      ...
      mensagem: 'As notificações estão funcionando perfeitamente!',
        ...
        <div className="card flex flex-col gap-4">
          ...
          {MODULOS.map(m => (
            <div key={m.key} className="flex items-center justify-between">
              ...
              <button
                onClick={() => setModulos(prev => ({ ...prev, [m.key]: !prev[m.key] }))}
