...
import {
  ...
  X, FileText, MessageSquare, Clock, Plus, Trash2
} from 'lucide-react'
...
export default function ConfiguracoesPage() {
  ...
  const [lembretes, setLembretes] = useState<ConfigLembrete[]>([])
  ...
  async function carregarDados() {
    ...
    const { data: configsLembretes, error: erroConfigsLembretes } = await supabase
      .from('lembretes_agendamento_config')
      .select('id, salao_id, antecedencia_minutos, ativo, titulo, mensagem, ordem')
      .eq('salao_id', profile!.salao_id!)
      .order('ordem', { ascending: true })

    if (!erroConfigsLembretes) {
      setLembretes(configsLembretes || [])
    } else {
      console.error('Erro ao carregar configurações de lembretes:', erroConfigsLembretes)
    }
  ...
  function adicionarLembrete() {
    setErroLembretes('')
    setLembretes(prev => [...prev, { antecedencia_minutos: 1440, ativo: true, titulo: 'Lembrete de agendamento', mensagem: 'Olá, {cliente}! Este é um lembrete do seu agendamento no {salao}, {data} às {hora}.', ordem: prev.length }])
  }

  function atualizarLembrete(index: number, campo: keyof ConfigLembrete, valor: any) {
    setLembretes(prev => prev.map((item, i) => i === index ? { ...item, [campo]: valor } : item))
  }

  async function removerLembrete(index: number) {
    const lembrete = lembretes[index]
    setErroLembretes('')
    if (lembrete?.id) {
      const { error } = await supabase.from('lembretes_agendamento_config').delete().eq('id', lembrete.id).eq('salao_id', profile!.salao_id!)
      if (error) { setErroLembretes('Não foi possível excluir este lembrete: ' + error.message); return }
    }
    setLembretes(prev => prev.filter((_, i) => i !== index).map((item, i) => ({ ...item, ordem: i })))
  }

  async function salvarLembretes() {
    setErroLembretes('')
    for (const lembrete of lembretes) {
      if (!Number.isFinite(Number(lembrete.antecedencia_minutos)) || Number(lembrete.antecedencia_minutos) <= 0) { setErroLembretes('A antecedência de cada lembrete deve ser maior que zero.'); return }
      if (!lembrete.titulo.trim()) { setErroLembretes('Informe um título para todos os lembretes.'); return }
      if (!lembrete.mensagem.trim()) { setErroLembretes('Informe uma mensagem para todos os lembretes.'); return }
    }
    setSalvandoLembretes(true)
    try {
      const salaoId = profile!.salao_id!
      const { data: atuais, error: erroAtuais } = await supabase.from('lembretes_agendamento_config').select('id').eq('salao_id', salaoId)
      if (erroAtuais) throw erroAtuais
      const idsMantidos = new Set(lembretes.filter(item => item.id).map(item => item.id))
      const idsParaExcluir = (atuais || []).map(item => item.id).filter(id => !idsMantidos.has(id))
      if (idsParaExcluir.length > 0) {
        const { error } = await supabase.from('lembretes_agendamento_config').delete().eq('salao_id', salaoId).in('id', idsParaExcluir)
        if (error) throw error
      }
      if (lembretes.length > 0) {
        const registros = lembretes.map((item, index) => ({ ...(item.id ? { id: item.id } : {}), salao_id: salaoId, antecedencia_minutos: Number(item.antecedencia_minutos), ativo: Boolean(item.ativo), titulo: item.titulo.trim(), mensagem: item.mensagem.trim(), ordem: index }))
        const { data: salvos, error } = await supabase.from('lembretes_agendamento_config').upsert(registros, { onConflict: 'id' }).select('id, salao_id, antecedencia_minutos, ativo, titulo, mensagem, ordem')
        if (error) throw error
        setLembretes((salvos || []).sort((a, b) => a.ordem - b.ordem))
      } else { setLembretes([]) }
      setLembretesSalvos(true)
      setTimeout(() => setLembretesSalvos(false), 2500)
    } catch (error: any) {
      console.error('Erro ao salvar lembretes:', error)
      setErroLembretes('Erro ao salvar os lembretes: ' + (error?.message || 'erro desconhecido'))
    } finally { setSalvandoLembretes(false) }
  }

  function formatarAntecedencia(minutos: number) {
    const total = Number(minutos)
    if (!Number.isFinite(total) || total <= 0) return 'Informe o tempo'
    if (total % 1440 === 0) { const dias = total / 1440; return `${dias} ${dias === 1 ? 'dia' : 'dias'} antes` }
    if (total % 60 === 0) { const horas = total / 60; return `${horas} ${horas === 1 ? 'hora' : 'horas'} antes` }
    return `${total} minutos antes`
  }

  function inserirVariavel(index: number, variavel: string) {
    const atual = lembretes[index]?.mensagem || ''
    atualizarLembrete(index, 'mensagem', atual + variavel)
  }
      ...
      <div className="px-4 py-4 flex flex-col gap-4 max-w-2xl mx-auto">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div><p className="font-bold text-gray-900 flex items-center gap-2"><Clock size={18} />Lembretes Automáticos de Agendamento</p><p className="text-xs text-gray-400 mt-1">Configure quando suas clientes devem receber um lembrete dos agendamentos confirmados.</p></div>
            <button onClick={adicionarLembrete} className="shrink-0 flex items-center gap-1 px-3 py-2 rounded-xl text-white text-xs font-bold" style={{ backgroundColor: cor }}><Plus size={15} />Adicionar</button>
          </div>
          <div className="bg-gray-50 rounded-xl p-3 text-xs text-gray-500">Você pode configurar mais de um lembrete para o mesmo agendamento, por exemplo: <strong>24 horas</strong> e <strong>2 horas</strong> antes.</div>
          {erroLembretes && <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2"><p className="text-red-600 text-xs font-semibold">{erroLembretes}</p></div>}
          {lembretes.length === 0 ? (
            <div className="border border-dashed border-gray-200 rounded-xl p-5 text-center"><Clock size={24} className="mx-auto text-gray-300 mb-2" /><p className="text-sm font-semibold text-gray-700">Nenhum lembrete configurado</p><p className="text-xs text-gray-400 mt-1">Adicione um lembrete para começar a configurar os avisos automáticos.</p><button onClick={adicionarLembrete} className="mt-3 px-4 py-2 rounded-xl text-white text-xs font-bold" style={{ backgroundColor: cor }}>+ Criar primeiro lembrete</button></div>
          ) : (
            <div className="flex flex-col gap-4">
              {lembretes.map((lembrete, index) => (
                <div key={lembrete.id || `novo-${index}`} className="border border-gray-200 rounded-2xl p-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-gray-800">Lembrete {index + 1}</p><p className="text-xs text-gray-400 mt-0.5">{formatarAntecedencia(lembrete.antecedencia_minutos)}</p></div><div className="flex items-center gap-2"><button onClick={() => atualizarLembrete(index, 'ativo', !lembrete.ativo)} className="relative w-12 h-6 rounded-full transition-colors shrink-0" style={{ backgroundColor: lembrete.ativo ? cor : '#d1d5db' }}><div className={'absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ' + (lembrete.ativo ? 'left-6' : 'left-0.5')} /></button><button onClick={() => removerLembrete(index)} className="p-2 rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-50" aria-label="Excluir lembrete"><Trash2 size={16} /></button></div></div>
                  <div><label className="text-xs font-semibold text-gray-600 block mb-1">Enviar com antecedência</label><input type="number" min={1} value={lembrete.antecedencia_minutos} onChange={e => atualizarLembrete(index, 'antecedencia_minutos', Number(e.target.value))} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" /><p className="text-[11px] text-gray-400 mt-1">Informe em minutos. Exemplos: 120 = 2 horas · 1440 = 24 horas · 2880 = 2 dias.</p></div>
                  <div><label className="text-xs font-semibold text-gray-600 block mb-1">Título da notificação</label><input type="text" value={lembrete.titulo} onChange={e => atualizarLembrete(index, 'titulo', e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" placeholder="Ex: Lembrete de agendamento" /></div>
                  <div><label className="text-xs font-semibold text-gray-600 block mb-1">Mensagem</label><textarea rows={4} value={lembrete.mensagem} onChange={e => atualizarLembrete(index, 'mensagem', e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm resize-none" placeholder="Olá {cliente}, lembrando do seu horário..." /><div className="flex flex-wrap gap-1.5 mt-2">{['{cliente}', '{salao}', '{data}', '{hora}', '{servico}'].map(variavel => <button key={variavel} type="button" onClick={() => inserirVariavel(index, variavel)} className="px-2 py-1 rounded-lg bg-gray-100 text-gray-600 text-[11px] font-medium">{variavel}</button>)}</div></div>
                  <div className="rounded-xl border border-gray-100 bg-gray-50 p-3"><p className="text-[11px] font-semibold text-gray-500 mb-1">Prévia</p><p className="text-sm font-bold text-gray-800">{lembrete.titulo || 'Sem título'}</p><p className="text-xs text-gray-600 mt-1 whitespace-pre-wrap">{lembrete.mensagem || 'Sem mensagem'}</p></div>
                </div>
              ))}
            </div>
          )}
          <button onClick={salvarLembretes} disabled={salvandoLembretes} className="w-full py-3 rounded-xl text-white font-semibold text-sm" style={{ backgroundColor: cor }}>{lembretesSalvos ? '✓ Lembretes Salvos!' : salvandoLembretes ? 'Salvando...' : 'Salvar Lembretes'}</button>
        </div>
        {/* 9. Notificações Push */}
