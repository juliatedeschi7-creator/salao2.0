...
import { ArrowLeft, Plus, Edit2, Trash2, Calendar, FileText, X, Save } from 'lucide-react'
...
function PacotesClienteContent() {
  ...
  const [formVender, setFormVender] = useState({
    pacote_id: '', sessoes_total: 1, data_expiracao: '',
    observacoes: '', vendido_por: '', preco_cobrado: ''
  })
  ...
  const [formAntigo, setFormAntigo] = useState({
    nome: '', sessoes_total: 1, sessoes_usadas: 0,
  ...
  const [formEditar, setFormEditar] = useState({
    sessoes_total: 1, sessoes_usadas: 0, status: 'ativo',
    data_expiracao: '', observacoes: ''
  })
  ...
  useEffect(() => {
    if (!loading && profile?.salao_id && clienteId) carregarDados()
  }, [loading, clienteId])
  ...
  async function carregarDados() {
    ...
    const { data: modelos } = await supabase
      .from('pacotes').select('*').eq('salao_id', profile!.salao_id!).eq('status', 'ativo')
    setModelosPacotes(modelos || [])

    const { data: funcs } = await supabase
      .from('profiles').select('id, nome').eq('salao_id', profile!.salao_id!).in('role', ['dono_salao', 'funcionario'])
  ...
  async function handleVender() {
    ...
    setFormVender({ pacote_id: '', sessoes_total: 1, data_expiracao: '', observacoes: '', vendido_por: '', preco_cobrado: '' })
  ...
  async function handleSalvarAntigo() {
    ...
    if (!error) { setModalAntigo(false); carregarDados() }
  ...
  function abrirEditar(p: ClientePacote) {
    ...
    setFormEditar({
      ...
      status: p.status,
      data_expiracao: p.data_expiracao ? p.data_expiracao.split('T')[0] : '',
  ...
  async function handleSalvarEdicao() {
    ...
    setModalEditar(false); setPacoteEditando(null)
    setSalvandoEditar(false); carregarDados()
  }

  async function excluirPacote(p: ClientePacote) {
    await supabase.from('cliente_pacotes').delete().eq('id', p.id)
    setConfirmandoExclusao(null); carregarDados()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="bg-white px-4 pt-12 pb-4 shadow-sm">
        <button onClick={() => router.back()} className="mb-3"><ArrowLeft size={22} className="text-gray-700" /></button>
        <h1 className="font-bold text-gray-900 text-xl">{cliente?.nome}</h1>
      ...
      <div className="px-4 py-4 flex flex-col gap-4">
        ...
        {pacotes.length === 0 ? (
          ...
          const temRegras = p.pacotes?.descricao && p.pacotes.descricao.length > 0
            ...
            <div key={p.id} className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-3">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {p.historico_manual && (
                    <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Cadastro manual</span>
                  )}
                </div>
                <div className="flex items-center gap-1.5 ml-2">
                  ...
                  <button onClick={() => setConfirmandoExclusao(p)}
          ...
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <select className="input-field" value={formVender.pacote_id}
                onChange={e => {
                  const modelo = modelosPacotes.find(m => m.id === e.target.value)
                  setFormVender(p => ({ ...p, pacote_id: e.target.value, sessoes_total: modelo?.sessoes || 1 }))
                }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input-field" type="date" value={formVender.data_expiracao}
                  onChange={e => setFormVender(p => ({ ...p, data_expiracao: e.target.value }))}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={handleVender} disabled={salvandoVender}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <input className="input-field" placeholder="Ex: 4 mãos e 2 pés"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className="input-field" type="date" value={formAntigo.data_compra}
                  onChange={e => setFormAntigo(p => ({ ...p, data_compra: e.target.value }))}
                  style={{ colorScheme: 'light' }} />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">Validade até</label>
                <input className="input-field" type="date" value={formAntigo.data_expiracao}
                  onChange={e => setFormAntigo(p => ({ ...p, data_expiracao: e.target.value }))}
                  style={{ colorScheme: 'light' }} />
              </div>
            </div>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={handleSalvarAntigo} disabled={salvandoAntigo}
                className="flex-1 py-3 rounded-2xl text-white font-medium" style={{ backgroundColor: cor }}>
      ...
      {modalEditar && pacoteEditando && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-sm text-gray-500">{pacoteEditando.historico_manual || pacoteEditando.pacotes?.nome || 'Pacote manual'}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input-field" type="number" min="0" max={formEditar.sessoes_total} value={formEditar.sessoes_usadas}
            ...
            <div>
              ...
              <input className="input-field" type="date" value={formEditar.data_expiracao}
                onChange={e => setFormEditar(p => ({ ...p, data_expiracao: e.target.value }))}
                style={{ colorScheme: 'light' }} />
            </div>

            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Observações</label>
              <input className="input-field" placeholder="Observações sobre este pacote"
                value={formEditar.observacoes} onChange={e => setFormEditar(p => ({ ...p, observacoes: e.target.value }))} />
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4 max-h-[85vh] overflow-y-auto">
            ...
            <p className="text-sm font-semibold" style={{ color: cor }}>
              {modalRegras.historico_manual || modalRegras.pacotes?.nome}
            </p>
            <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">
              {modalRegras.pacotes?.descricao}
            </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-gray-500">
              O pacote <span className="font-semibold">{confirmandoExclusao.historico_manual || confirmandoExclusao.pacotes?.nome}</span> de {cliente?.nome} será removido permanentemente.
            </p>
...
export default function PacotesClientePage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-gray-300 border-t-transparent rounded-full animate-spin" /></div>}>
      <PacotesClienteContent />
    </Suspense>
  )
}
