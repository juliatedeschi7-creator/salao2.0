  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gray-900 px-4 py-5 flex items-center gap-3">
        ...
        {usuarios.filter(u => !u.aprovado && !u.excluido).length > 0 && (
          <span className="bg-yellow-400 text-gray-900 text-xs px-2 py-1 rounded-full font-bold">
            {usuarios.filter(u => !u.aprovado && !u.excluido).length} pendentes
          </span>
        )}
      </div>
          ...
          <div key={u.id} className="card flex flex-col gap-3">
            ...
            {u.excluido ? (
              <button onClick={() => reativarUsuario(u)}
                ...
                <RotateCcw size={16} />Reativar usuario
              </button>
            ) : !u.aprovado ? (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white w-full rounded-t-3xl p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-gray-500">
              {confirmandoExclusao.nome} sera removido do sistema, mas podera ser reativado a qualquer momento na aba "Excluidos".
            </p>
